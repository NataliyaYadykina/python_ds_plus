...
# который инициализирует атрибуты rows и cols,
# а также создает двумерный список data
# размером rows x cols и заполняет его нулями.
# __str__(self): Метод, возвращающий строковое представление матрицы.
# Возвращаемая строка представляет матрицу,
# где элементы разделены пробелами,
...
# Проверяет, что количество столбцов в первой матрице
# равно количеству строк во второй матрице.
            ...
            for i in range(self.rows):
                for j in range(other.cols):
                    for k in range(len(other.data[0])):
                        new_matrix.data[i][j] += self.data[i][k] * \
                            other.data[k][j]
            return new_matrix
        return None
