...
# который имеет атрибут name, представляющий имя животного.
...
# Fish имеет атрибут max_depth (максимальная глубина обитания)
# и метод depth, который возвращает категорию глубины рыбы
# (мелководная, средневодная, глубоководная)
# в зависимости от значения max_depth.
# Если максимальная глубина обитания рыбы
# (max_depth) меньше 10,
...
# в зависимости от веса. Если вес объекта меньше 1,
# то он относится к категории "Малявка".
# Если вес объекта больше 200,
# то он относится к категории "Гигант".
...
# Класс-фабрика должен иметь метод create_animal,
...
# представляющих параметры для конкретного типа животного.
# Количество и типы аргументов могут различаться
# в зависимости от типа животного.
# Метод create_animal должен создавать и возвращать
# экземпляр животного заданного типа с переданными параметрами.
# Пример
# На входе:
...
class AnimalFactory:
    ...
    def __init__(self) -> None:
        AnimalFactory.count += 1

    def create_animal(animal_type, *args):
        name, param = args[0], args[1]
        if animal_type == 'Bird':
            return Bird(name, param)
        elif animal_type == 'Fish':
            return Fish(name, param)
        elif animal_type == 'Mammal':
            return Mammal(name, param)
        else:
...
if __name__ == "__main__":
    ...
    animal1 = AnimalFactory.create_animal('Bird', 'Орел', 200)
    animal2 = AnimalFactory.create_animal('Fish', 'Лосось', 50)
    animal3 = AnimalFactory.create_animal('Mammal1', 'Слон', 5000)
